...
export default function LoginPage() {
  ...
  return (
    <main className="mx-auto flex min-h-[60vh] w-full max-w-md flex-col justify-center px-6 py-16">
      <h1 className="text-2xl font-black tracking-tight text-[var(--color-text)]">Se connecter</h1>
      <p className="mt-2 text-sm text-[var(--color-muted)]">
        Pas de mot de passe. Entrez votre e-mail, on vous envoie un lien de connexion.
      </p>

      {sent ? (
        <div data-testid="login-sent" className="mt-8 rounded-xl border border-[var(--color-border)] p-5 text-sm text-[var(--color-text)]">
          <p className="font-semibold">Consultez vos e-mails</p>
          <p className="mt-1 text-[var(--color-muted)]">
            Si un compte est possible pour <strong>{email}</strong>, un lien de connexion vient d'être envoyé. Il expire dans 15 minutes.
          </p>
        </div>
      ) : (
      ...
      <p className="mt-6 text-xs text-[var(--color-muted)]">
        Vous pouvez aussi continuer sans compte — le mode invité reste disponible.
      </p>
    </main>
  );
}
