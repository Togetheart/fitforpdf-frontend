...
export default function useConversion({ quota }) {
  ...
  // Branding/logo defaults ON for everyone so the logo upload + size control are
  // DISCOVERABLE — if the branding section were hidden by default you'd never learn you
  // can add or replace a logo. Paid users who want a white-label PDF just toggle it off;
  // that choice is remembered (touched ref) and the server still has the final say.
