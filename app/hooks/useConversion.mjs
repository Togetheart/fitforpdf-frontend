...
export default function useConversion({ quota }) {
  ...
  // Paid plans are white-label by default (clean PDF, no "Generated by FitForPDF");
  // free plans keep branding (it's forced on server-side anyway). Applied once the
  // plan is known — unless the user has manually toggled it, in which case their
  // choice wins. The server still has the final say per entitlement.
  const brandingTouchedRef = useRef(false);
  useEffect(() => {
    if (brandingTouchedRef.current) return;
    const plan = String(quotaPlanType || '').toLowerCase();
    const isPaid = plan !== '' && plan !== 'free';
    setIncludeBranding(!isPaid);
  }, [quotaPlanType]);
  const handleBrandingChange = (next) => {
    brandingTouchedRef.current = true;
    setIncludeBranding(next);
  };
  ...
  return {
    ...
    // options
    includeBranding,
    setIncludeBranding: handleBrandingChange,
    truncateLongText,
    setTruncateLongText,
    retainSourceConsent,
