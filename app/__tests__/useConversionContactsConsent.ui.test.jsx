...
function Harness({ consent }) {
  ...
  React.useEffect(() => {
    ...
    if (consent) conversion.setContactsConsent(true);
    setReady(true);
  }, [conversion, consent]);
  return (
...
describe('useConversion, contacts_consent in render FormData', () => {
  test('default (no consent) → contacts_consent="0"', async () => {
    ...
    expect(getRenderFormData(mock.calls).get('contacts_consent')).toBe('0');
    mock.restore();
  });

  test('consent on → contacts_consent="1"', async () => {
    const mock = mockFetch(() => createPdfResponse());
    render(<Harness consent />);
    fireEvent.click(await screen.findByRole('button'));
    await waitFor(() => { expect(getRenderFormData(mock.calls)).not.toBeNull(); });
    expect(getRenderFormData(mock.calls).get('contacts_consent')).toBe('1');
    mock.restore();
  });
});
