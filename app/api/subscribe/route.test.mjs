...
test('POST /api/subscribe requires CLEAN_SHEET_API_URL', async () => {
  const restoreEnv = setupEnv(undefined);
  const req = new Request('https://www.fitforpdf.com/api/subscribe', {
    method: 'POST',
...
test('POST /api/subscribe forwards email payload to upstream', async () => {
  const restoreEnv = setupEnv('https://api.fitforpdf.neatexport.local');
  const fetchMock = withMockFetch(({ url, options }) => {
    assert.equal(url, 'https://api.fitforpdf.neatexport.local/v1/subscribe');
  ...
  assert.equal(payload, JSON.stringify({ ok: true }));
...
test('POST /api/subscribe returns 502 when upstream fails', async () => {
  const restoreEnv = setupEnv('https://api.fitforpdf.neatexport.local');
